...
- Source implementation is committed and published through `22463194` on `origin/agent/linux-cuda-nccl`. The local remotes are normalized as `upstream=exo-explore/exo` and `origin=ldyeax/exo`; push this branch after each verified commit.
...
- Remaining limitations before general NVIDIA support: ordered explicit GPU subsets are supported for NCCL Tensor placement, but the strict two-host live proof harness still assumes the SmolLM 2+1 inventory and must be generalized before TP2 ladder runs; KV/workspace/prefix-cache VRAM is not modeled; GPU enumeration still requires NVML even though locality now comes from sysfs; SGLang external-process lifecycle, API proxying, patched-runtime capability collection, and live SM86 GLM/NSA validation are not implemented; and live multi-GPU dashboard/hardware validation remains pending.
...
4. Staging is complete for `mlx-community/SmolLM2-135M-Instruct-8bit@0f0d9b8218915bc34d401e1a340b8c049d300d5e`. The non-performance receipt at `/var/lib/exo/benchmarks/smollm2-stage-20260718-v1` completed normally with verified cleanup and identical 22-file SHA-256 manifests on both hosts. The indexed weights are 142,955,136 bytes. Revision-pinned destinations are `dwagon:/var/lib/exo/models/mlx-community--SmolLM2-135M-Instruct-8bit--0f0d9b8218915bc34d401e1a340b8c049d300d5e` and `fwuff:/mnt/sanic/exo/models/mlx-community--SmolLM2-135M-Instruct-8bit--0f0d9b8218915bc34d401e1a340b8c049d300d5e`.
   Its deterministic TP1 oracle is complete; use completion SHA-256 `3f466ee4633b7a26654a26badb971ab41a08908ed4adf4ad9a5277a498c596ca`. Next run Tensor=3 with `MlxNccl` across dwagon's two GPUs and fwuff's GPU. Its hidden size 576, 9 attention heads, 3 KV heads, and MLP width 1536 are all divisible by three. Use TP3 as a scheduler, multi-runner lifecycle, and collective-correctness proof rather than a performance result. The dashboard can request this topology by selecting NCCL Tensor, two minimum nodes, and `Use all available GPUs`. Hold the `/ai/coordinate.md` lease for each live run and obey the required cooldown between runs.
