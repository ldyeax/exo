...
- The upstream Flash source base is KTransformers `8e46e5896c3d993a1285052f2618f5a9f01882d4` (`v0.6.3.post1`) with embedded SGLang `5d6bef9f61637aaeaf047bf8209def2af3eaa83f`; no Ornith source patch is needed for the first BF16 hybrid smoke. Exo now carries exact mail patches under `scripts/patches/sglang_kt/` and `scripts/prepare_sglang_kt_source.py` reproduces clean admitted revisions KTransformers `a4b0c45aa6f5f8d48f28b4f07d7591f0fb8960a8` and SGLang `449c59f07752189baf724630c6799c28736cee87` from those bases. The SGLang patch imports/registers `kt_ep_wrapper` fatally before `Glm4MoeLiteForCausalLM` construction and emits structured coverage only after exactly `model.layers.1..46.mlp.experts` use wrapper ID `kt_ep` with valid global and per-layer expert masks. Twelve dependency tests and nine Exo bootstrap tests cover clean-base, uninitialized-submodule, initialized-base-only, idempotent, unrelated dirty-submodule, dirty-source, and corrupt-patch paths; the runtime execution receipt remains required before live admission.
