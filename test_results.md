...
- Latest published proof-harness source: clean commit
  `ab2752135f7df6307d3a98c2ece0477381a52254`.
...
- Next work: establish an untuned four-resident serving baseline, then adapt the
  pinned runtime's fused-MoE tuner for GLM's masked global routes and create
  immutable RTX 3090 E=4 gate/up and down-kernel configurations. Keep correctness
  and serving-performance receipts separate.
...
| Contained GLM-4.7 serving-baseline admission phase | **PASS (software)** | The existing lease, immutable deployment, delegated cgroup, NUMA/GPU/HCA preflight, kernel validation, model validation, and ownership-safe cleanup harness can now select the exact `serving_baseline` launch mode. It requires 1-4 resident GPU experts and remains an admission-only run with `performance_comparable=false`. The harness/generator slice passed 96 tests; strict source Basedpyright, Ruff, and formatting pass. A fresh leased hardware run is pending. |
