...
- The first one-resident-GPU-expert control, hybrid1 v1, loaded the exact model
  and wrappers but failed the repeated combined CPU/GPU layer-one hash before
  short forward or receipt publication. A pinned-runtime audit found that the
  validator reused one dispatch input even though Triton's fused MoE mutates it
  in place. The corrected probe now clones a pristine input and dispatch for
  every invocation while retaining exact repeat admission.
...
- Next work: rerun the corrected immutable one-resident AMX-BF16/RTX-3090
  correctness gate, then run the four-resident gate. Keep each receipt separate
  and do not treat short-forward validation timings as serving-performance
  measurements.
...
and must not be reused. Prepare fresh corrected one- and four-resident-GPU-
expert mixed proofs against the same pinned runtime. V4's
