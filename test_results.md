...
| GLM-4.7 Flash 30B-A3B | `glm47-tp2-local-dwagon-20260720-v2` | dwagon; PP1/TP2; 2x RTX 3090 over NV4; 112 physical cores/2 AMX pools; E44; CPU performance policy | 5.595086874* | 6.787595236 | **PASS (diagnostic)**; 5.69% above TP2 E40 but 16.7% below PP2 E44 |
...
| GLM-4.7 Flash 30B-A3B | `glm47-tp2-local-dwagon-20260720-v1` | dwagon; PP1/TP2; 2x RTX 3090 over NV4; 112 physical cores/2 AMX pools; E40; CPU performance policy | 5.340388983* | 6.422166737 | **PASS (diagnostic)**; 21.2% slower than PP2 E44; 0.21 GB KV/rank at 4,096 tokens |
...
- Latest batch-one kernel result: controlled OLMoE kernel A/B runs raised
  ...
  Every A/B pair had exact output hashes, exact sanity, unforced cleanup, and
  restored CPU policy; each tuned run also had four authenticated config-load
  markers with no fallback. Tuned EP2 remains 8.524% slower than tuned EP1 at
  batch-one decode. EP2 is true
  expert-ID/weight sharding through SGLang's standard masked-local dispatcher
  and output all-reduce, not token A2A.
  Both modes passed exact one-token sanity and all prefill-heavy outputs matched.
  The receipt-backed 131-token probe found IDs 139 and 1769 at exactly equal
  logprob in both modes: `-4.532527446746826` under EP1 and
...
- Latest GLM topology result: local PP1/TP2 v2 raised residency from E40 to
  ...
  6.787595236 decode and 5.595086874 prefill-heavy output tok/s, gains of
  5.690% and 4.769% over TP2 E40 but still 16.725% and 17.144% below PP2 E44.
  Both CPU packages averaged about 329.7 W during decode with no throttle
  counts. E44 raised each rank's weight load to 20.19-20.21 GB and left only
  2.85-2.87 GB before its 0.21 GB local KV allocation. Thus the reported
  approximately 2 GB spare-memory condition is plausible at high residency,
  but it is weight/expert baggage rather than an unavoidable full-model KV
  cache. Local PP2 v9 remains the best reproducible native hybrid result at
  8.150791955 decode and 6.752822778 prefill-heavy output tok/s. These runs
  remain diagnostic while NCCL INFO and phase telemetry are enabled.
...
- Next work: tune the exact RTX 3090 Triton MoE shapes, sweep request
  concurrency, then repeat the matched
...
| OLMoE tied-logit diagnostic | **PASS (software)** | Commit `14302708` adds a capture-only, exact-context logit probe with arbitrary candidate IDs, top-k consistency, greedy-argmax, size, compression, deadline, and cleanup bounds. The final focused slice passed 90 tests; targeted strict type checking passed for the new client, Ruff and `git diff --check` passed, and final independent review found no remaining high- or medium-severity issue. |
...
| Transactional CPU performance policy | **PASS (software)** | Commit `59a989ec` serializes host-wide policy changes with `flock`, durably journals exact original topology/governor/EPP state before mutation, recovers stale transactions, verifies active/final topology, restores exact values, and binds throttle/RAPL/temperature evidence. All 18 fake-sysfs/process tests passed, including live-owner, crash recovery, topology drift, partial writes, and interrupted hard-link publication; two independent review passes found no remaining issue; Ruff and formatting passed. |
...
| GLM-4.7 Flash local TP2 v1 | **PASS (diagnostic)** | `/var/lib/exo/benchmarks/glm47-tp2-local-dwagon-20260720-v1`; the first native hybrid PP1/TP2 run used ordered NUMA-matched RTX 3090s, all 112 physical cores, two AMX CPUInfer pools, E40, and the CPU performance policy. Exact sanity passed; 5.340388983 prefill-heavy output tok/s and 6.422166737 decode tok/s were 20.916% and 21.208% below PP2 E44. Decode samples were 6.422166737/6.777258954/6.249091421 tok/s. Both ranks used NCCL P2P/IPC, loaded 18.57-18.59 GB weights with about 4.5 GB remaining, and allocated 0.21 GB KV cache for 4,096 tokens. Decode package power averaged 329.77/329.77 W under 330 W PL1, temperatures peaked at 78/80 C, hardware throttle counts remained zero, all CPU policies restored, both GPUs released to 1 MiB, and cleanup was complete. Receipt content SHA `272bf9e70e5cf1037d5eb5f4473dd71f7f0098c6abc14599bf54b08a16d9f5e9`; file SHA `03a5cbc6997b1faf2f2a2edd87a8d7d7f3c37387e0736894217d028bde098c05`. |
| GLM-4.7 Flash local TP2 v2 | **PASS (diagnostic)** | `/var/lib/exo/benchmarks/glm47-tp2-local-dwagon-20260720-v2`; the controlled E40-to-E44 comparison retained the exact topology and CPU performance policy while raising memory fraction to 0.95. Exact sanity passed; 5.595086874 prefill-heavy output tok/s and 6.787595236 decode tok/s improved 4.769% and 5.690% over TP2 E40, but remained 17.144% and 16.725% below PP2 E44. Decode samples were 6.787595236/6.945261938/6.742052857 tok/s. Each rank loaded 20.19-20.21 GB weights, retained 2.85-2.87 GB, and allocated 0.21 GB KV cache for 4,096 tokens. Decode package power averaged 329.69/329.76 W under 330 W PL1, temperatures peaked at 80/80 C, hardware throttle counts remained zero, all CPU policies restored, both GPUs released to 1 MiB, and cleanup was complete. Receipt content SHA `7c21777070c2a87048e1b150664f77a4c5c1f1b676d630c6ad531aa323ab05ba`; file SHA `0bd9d2d68609f9a7a5a33241f465e7c895654656c7ec1d1ee2d253913c73ba65`. |
...
| OLMoE local EP2 timed v1 | **PASS (diagnostic)** | `/var/lib/exo/benchmarks/olmoe-ep2-local-dwagon-20260720-v1`; true expert-ID/weight sharding reached 202.942340498 prefill-heavy end-to-end output and 270.100859149 generation-window decode tok/s. All prefill outputs exactly match EP1. The decode first differs at an exact-logprob tie on generated token 4 (`139` versus `1769`), so the semantic concern is cleared but its 12.53% timing delta still includes different later expert routes. Cleanup was unforced and policy restored. Receipt SHA `79d803e380967ca268a3ad6e845c1c9beab4bd9117bb1e2c43910ea5d9e41e52`; log SHA `38b50e54289c4fdee27a4ba31b1e23be6760029e8dad3e22878735079f3434aa`. |
| OLMoE EP1 tied-logit capture v1 | **PASS** | `/var/lib/exo/benchmarks/olmoe-ep1-logit-capture-dwagon-20260720-v1`; committed source `14302708` reproduced the exact 131-token divergence context. Candidate IDs 139 and 1769 both had logprob `-4.532527446746826`; generated ID 139 is therefore a valid greedy tie resolution. Exact completion sanity, NUMA admission, unforced cleanup, and CPU-policy restoration passed. Receipt SHA `6151e37a5952a97c49b99a4035f39d3275bfb904298df7f8936db9b7c0339a8d`; log SHA `af223131d7cc5c80ac60bae555518609e37f312423533d290c18fe99c8e8950b`; capture SHA `72137d76745f5846fe434eaadd6233043fde2f5f000fdb1aa3d63fad97676881`. |
| OLMoE EP2 tied-logit capture v1 | **EXPECTED FAIL (setup)** | `/var/lib/exo/benchmarks/olmoe-ep2-logit-capture-dwagon-20260720-v1`; the contract-required port 62630 was still in TCP `TIME_WAIT` immediately after EP1. The preflight failed before model launch or GPU allocation, and CPU policy restored exactly. Receipt SHA `b5dbef3a82454b69e7cbc50c6b8407a7c6272c0f0df7a003a01fe8eb4f66b972`. |
| OLMoE EP2 tied-logit capture v2 | **PASS** | `/var/lib/exo/benchmarks/olmoe-ep2-logit-capture-dwagon-20260720-v2`; after the kernel timeout cleared, the identical context gave both candidate IDs 139 and 1769 logprob `-4.558165073394775`. Exact equality in both EP modes clears the token-4 semantic concern: later output divergence begins at a legitimate greedy tie. Exact sanity, expert identity, NUMA admission, unforced cleanup, and CPU-policy restoration passed. Receipt SHA `6d7643ac06de9f145a616ee22869fd4e14835c258d72ccca379f806da27c153d`; log SHA `eb5145305f31244f89f2d96137f18f8d246df297472013168572811f2640871b`; capture SHA `2da2ba24d5efb3e51fdeb0a851787357829c24c57093683f519a3449f2e6a753`. |
| OLMoE paired stage contract v2 | **PASS (artifact)** | `/var/lib/exo/benchmarks/olmoe-stage-contract-dwagon-20260720-v2/olmoe-stage-contract.json`; full-rehashes the exact 13,838,721,960-byte snapshot and binds the committed-source EP1/EP2 captures with exact one-token ` 42` equality. Contract SHA `d54c92f60483f3786c85a2e92f2dbb8d3898bb924902fd9a87c3093912976881`; snapshot canonical SHA `b12f837d25b4a085e7a9dcd64fe35f6d7414c380fceb3720b1cea9d4bcfb5abc`. |
| OLMoE RTX 3090 tuner attempt 1 | **EXPECTED FAIL (unreceipted development)** | The first live EP1 M=1 setup stopped before timing because BF16 expert output multiplied by FP32 route weights produced an FP32 Torch reference. No candidate directory or durable receipt was published. Commit `4dd86dda` adds the serving-equivalent BF16 cast. |
| OLMoE RTX 3090 tuner attempt 2 | **EXPECTED FAIL (unreceipted development)** | The corrected sweep reached EP1 M=1024, where SGLang's own fallback differed from the Torch reference by one BF16 step (`0.03125` at magnitude about `5.1`) while relative L1 remained about `0.0012`. No candidate directory or durable receipt was published. The measured gate is now max-absolute `0.04` plus relative-L1 `0.02`. |
| OLMoE RTX 3090 tuner v1 | **PASS (artifact)** | `/var/lib/exo/benchmarks/olmoe-moe-tuning-dwagon-20260720-v1`; exact EP1/EP2 shapes, three route seeds, six candidates, four anchors, repeat-exact numerical evidence, and five fallback-candidate-fallback samples. Only M=1 cleared the 5% gate. Manifest SHA `8674a01e0754f472b345c5bcd92b90e03271d6c404f4027ac240c35873168e99`; receipt SHA `067784d4395d682a590549162553f3af38799c173d013219330a7058388d1947`; tuner SHA `5ab443031117535b2a0336af231f1458b3f4bfe8cfaefe1a1a2d7fedb622650b`. |
| OLMoE EP1 kernel A/B baseline v1 | **EXPECTED FAIL (admission)** | `/var/lib/exo/benchmarks/olmoe-ep1-local-dwagon-kernel-ab-base-20260720-v1`; failed before model launch because the immutable v2 captures bind historical committed harness `14302708`. Exact-receipt-scoped historical admission was added without rewriting or recapturing evidence. Receipt SHA `16b7dde4081405e4daf44cc5c0750a9136e79c0ea963c2638a17abed2d0e972d`. |
| OLMoE EP1 kernel A/B baseline v2 | **PASS (controlled A/B baseline)** | `/var/lib/exo/benchmarks/olmoe-ep1-local-dwagon-kernel-ab-base-20260720-v2`; exact sanity and outputs, 225.678633105 prefill-heavy E2E and 307.030114217 decode-window tok/s, verified fallback logs, unforced cleanup, and policy restoration. Receipt SHA `e0b4bc375b2c7a4e7b8080008fb08a778ba7cd7a87d0125e75e71de44fd060c6`; log SHA `fbb01a730d865a57d86ce14b2474385ec0bafbf4e580843ccb5815cefca016bf`. |
...
- `allenai/OLMoE-1B-7B-0924` revision
  ...
  `/var/lib/exo/benchmarks/olmoe-stage-contract-dwagon-20260720-v2/olmoe-stage-contract.json`
  has SHA `d54c92f60483f3786c85a2e92f2dbb8d3898bb924902fd9a87c3093912976881`
...
- EP2 was 11.534% slower in prefill generation-window throughput, 10.021%
  ...
  within each timed mode but first differs at generated token 4 after common
  IDs `[431, 3056, 209]`. The committed-source teacher-forced probe found IDs
  139 and 1769 at exactly equal logprob in both modes, so this is legitimate
  tie resolution under different reduction/kernel geometry, not a semantic EP
  defect. Later decode tokens follow different expert routes, so those
  throughput values remain diagnostic rather than an exact-output A/B.
...
- The bounded RTX 3090 sweep selected only M=1 overrides. Controlled serving
  A/B then improved EP1 decode by 8.214% and EP2 decode by 12.581%, with exact
