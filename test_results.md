...
- Latest published proof-harness source: clean commit
  `460339f3ccf7c2878f06db062b1f20bc60475f85`.
...
- Latest hybrid result: hybrid1 v2 is the first reportable one-resident-GPU-
  expert GLM-4.7 Flash SGLang-KTransformers admission. It binds the immutable
  62.4 GB BF16 checkpoint, AMX-BF16 kernel receipt, one RTX 3090 expert per
  routed layer, exact layers 1-46, deterministic CPU/GPU component and merged
  oracles, and real extend/decode logits. It derives only the seven exact mixed-
  route capabilities and completed cleanly. This is a correctness proof, not a
  performance result.
...
- Next work: run the four-resident AMX-BF16/RTX-3090 correctness gate. Keep its
  receipt separate and do not treat short-forward validation timings as serving-
...
must remain immutable. Prepare a fresh four-resident-GPU-expert mixed proof
against the same pinned runtime. V4's
