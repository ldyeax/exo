#!/usr/bin/env python3
"""Install Exo's receipt-bound SGLang-KTransformers wheels into an overlay runtime.
...
absolute ``.pth`` entry and shadows only the three receipt-bound runtime wheels.
The admitted GLM-4.7 revisions remain the default; experimental builds require
both exact source revisions at preflight and installation time.
"""
...
HEX_GIT_REVISION = re.compile(r"[0-9a-f]{40}", re.ASCII)
...
def _require_git_revision(value: str, description: str) -> str:
    if HEX_GIT_REVISION.fullmatch(value) is None:
        raise RuntimeInstallError(
            f"{description} must be an exact lowercase 40-character Git object ID"
        )
    return value
...
def _embedded_provenance_contents(
    ...
    return (
        ...
        f'SGLANG_REVISION = "{sglang_revision}"\n'
    ).encode("ascii")
        ...
        with ZipFile(wheel_path) as wheel:
            ...
            if provenance_package is not None:
                ...
                if wheel.read(provenance_path) != _embedded_provenance_contents(
                    expected_ktransformers_revision,
                    expected_sglang_revision,
                ):
                    raise RuntimeInstallError(
...
def observe_runtime_build(
    build_receipt: Path,
    *,
    expected_ktransformers_revision: str = GLM_4_7_FLASH_KTRANSFORMERS_REVISION,
    expected_sglang_revision: str = GLM_4_7_FLASH_SGLANG_REVISION,
) -> RuntimeBuildObservation:
    _require_git_revision(
        expected_ktransformers_revision,
        "expected KTransformers revision",
    )
    _require_git_revision(
        expected_sglang_revision,
        "expected SGLang revision",
    )
    ...
    if ktransformers_revision != expected_ktransformers_revision:
        raise RuntimeInstallError(
            f"build receipt KTransformers revision is {ktransformers_revision}, "
            f"expected {expected_ktransformers_revision}"
        )
    if sglang_revision != expected_sglang_revision:
        raise RuntimeInstallError(
            f"build receipt SGLang revision is {sglang_revision}, expected "
            f"{expected_sglang_revision}"
        )
...
def plan_runtime_install(
    ...
    *,
    ...
    build = observe_runtime_build(
        build_receipt,
        expected_ktransformers_revision=expected_ktransformers_revision,
...
def execute_runtime_install(
    ...
    *,
    ...
    current_build = observe_runtime_build(
        Path(plan.build.receipt_path),
        expected_ktransformers_revision=plan.build.ktransformers_revision,
        expected_sglang_revision=plan.build.sglang_revision,
    )
...
def _git_revision_argument(raw: str) -> str:
    if HEX_GIT_REVISION.fullmatch(raw) is None:
...
def _parse_arguments() -> _CliArguments:
    ...
    parser.add_argument(
        ...
        type=_git_revision_argument,
        help=(
            "Exact experimental SGLang commit. Must be supplied together with "
            "--ktransformers-revision."
        ),
    )
    parser.add_argument(
...
def main() -> int:
    ...
    try:
        ktransformers_revision, sglang_revision = _selected_revisions(
