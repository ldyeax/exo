...
def make_receipt(*, cpu_control: bool) -> JsonObject:
    ...
    resident_ids: list[JsonValue] = [] if cpu_control else [0, 1]
    global_mask_sha256 = calculate_glm_4_7_flash_expert_mask_sha256(
        tuple(tuple(int(expert_id) for expert_id in resident_ids) for _ in range(46))
    )
    selected_ids: list[JsonValue] = [4, 5, 6, 7] if cpu_control else [0, 1, 4, 5]
    cpu_ids: list[JsonValue] = [4, 5, 6, 7] if cpu_control else [4, 5]
...
def test_rejects_mutated_execution_evidence(
    ...
    numerical = _object_field(probe, "numerical")
