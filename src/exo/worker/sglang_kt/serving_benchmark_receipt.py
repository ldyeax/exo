...
WARM_SERVING_RUN_RECEIPT_SCHEMA_VERSION = 3
...
@final
class WarmServingRunReceiptV3(_StrictModel):
    ...
    @model_validator(mode="after")
    def validate_complete_run(self) -> "WarmServingRunReceiptV3":
...
class SglangKtWarmServingRunReceiptObservation(_StrictModel):
    ...
    receipt: WarmServingRunReceiptV3
...
def _validate_warm_serving_run_receipt_contents(
    ...
    return WarmServingRunReceiptV3.model_validate_json(contents)


def canonicalize_sglang_kt_warm_serving_run_receipt(payload: object) -> bytes:
    """Validate a warm-serving v3 receipt and return canonical JSON bytes."""
...
def load_sglang_kt_warm_serving_run_receipt(
    ...
    *,
    ...
    """Load a stable v3 receipt and bind it to an independent identity."""
